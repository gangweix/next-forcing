...
Training expects a pretrained model and a prepared LeRobot latent dataset. The
dataset root must contain `empty_emb.pt` and the converted dataset directories.

Configure the paths with environment variables:

```bash
export NEXT_FORCING_PRETRAINED_MODEL_PATH=/path/to/pretrained/model
...
Configure the inference checkpoint and RoboTwin repository:

```bash
export NEXT_FORCING_MODEL_PATH=/path/to/next-forcing-posttrain-robotwin
...
script/          Post-training launcher, dataset conversion, checkpoint conversion
